...
function loadParagraph() {
  ...
  const randomNum = Math.floor(Math.random() * paragraph.length);
  typingText.innerHTML = ""; // Clear previous text

  for (const char of paragraph[randomNum]) {
...
function initTyping(event) {
  ...
  if (event.inputType === 'deleteContentBackward') {
    ...
    char[charIndex].classList.remove("activeChar");
    if (charIndex > 0) {
      charIndex--;
      ...
      char[charIndex].classList.add("activeChar");
    }
    ...
    if (charIndex > 0) {
      char[charIndex - 1].classList.remove("activeChar");
    }
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Tab") {
    document.addEventListener(
      ...
      { once: true }
    ); // Ensures this listener is only called once
  }
});
